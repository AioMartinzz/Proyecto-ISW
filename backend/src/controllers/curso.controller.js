"use strict";
import { createCursoService, deleteCursoService, updateCursoService } from "../services/curso.service.js";
...
export async function createCurso(req, res) {
  try {
    ...
    const [curso, error] = await createCursoService(nombre, nivel, año, profesorJefeId);
...
export async function deleteCurso(req, res) {
  try {
    ...
    res.status(200).json({ message: "Curso eliminado exitosamente", data: curso });
  } catch (error) {
...
export async function updateCurso(req, res) {
  try {
    ...
    const [curso, error] = await updateCursoService(id, { nombre, nivel, año, profesorJefeId });
    if (error) return res.status(404).json({ message: error });

    res.status(200).json({ message: "Curso modificado exitosamente", data: curso });
  } catch (error) {
